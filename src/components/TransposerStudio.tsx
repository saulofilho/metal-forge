...
export const TransposerStudio: React.FC<TransposerStudioProps> = ({ onNavigateToAmp, onLoadAmpPreset }) => {
  const [activeModule, setActiveModule] = useState<TransposerSubModule>("scraper");
  ...
  // Load a popular song preset into manual transposer
...
${transposedData.dropCFretboardTips.map((t) => "- " + t).join("\n")}`;
  ...
  return (
    <div id="transposer-studio" className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 bg-[#141416] p-2 rounded-2xl border border-[#222226]">
        <div className="flex items-center gap-1.5 flex-wrap">
          <button
            id="tab-btn-riff-scraper"
          ...
          <button
            id="tab-btn-style-converter"
            onClick={() => setActiveModule("style-converter")}
            className={`px-4 py-2 rounded-xl text-xs font-mono font-bold transition-all flex items-center gap-2 cursor-pointer ${
              activeModule === "style-converter"
                ? "bg-[#CCFF00] text-black shadow-lg shadow-[rgba(204,255,0,0.2)]"
                : "text-gray-400 hover:text-gray-200 hover:bg-[#1D1D21]"
            }`}
          >
            <Flame className="w-4 h-4 fill-current" />
            <span>Style Converter (Metalize Anything)</span>
          </button>

          <button
            id="tab-btn-manual-transposer"
            onClick={() => setActiveModule("transposer")}
            className={`px-4 py-2 rounded-xl text-xs font-mono font-bold transition-all flex items-center gap-2 cursor-pointer ${
              activeModule === "transposer"
                ? "bg-[#CCFF00] text-black shadow-lg shadow-[rgba(204,255,0,0.2)]"
                : "text-gray-400 hover:text-gray-200 hover:bg-[#1D1D21]"
            }`}
          >
          ...
          <button
            id="tab-btn-fretboard-lib"
            onClick={() => setActiveModule("fretboard")}
            className={`px-4 py-2 rounded-xl text-xs font-mono font-bold transition-all flex items-center gap-2 cursor-pointer ${
              activeModule === "fretboard"
                ? "bg-[#CCFF00] text-black shadow-lg shadow-[rgba(204,255,0,0.2)]"
                : "text-gray-400 hover:text-gray-200 hover:bg-[#1D1D21]"
            }`}
          >
            <Music className="w-4 h-4" />
            <span>Drop C Voicing Library</span>
          </button>
        </div>

        <div className="hidden sm:flex items-center gap-2 px-3 py-1 bg-[#0A0A0B] rounded-xl border border-[#222226] text-[11px] font-mono text-gray-400">
          <span className="w-2 h-2 rounded-full bg-[#CCFF00] animate-pulse" />
          <span>Tuning: <strong className="text-gray-200">C-G-C-F-A-D</strong></span>
        </div>
      </div>

      {/* Module 1: Riff Scraper */}
      {activeModule === "scraper" && (
        <RiffScraper
          onApplyTransposition={(data, title, art) => {
      ...
      {/* Module 2: Style Converter ("Metalize Anything") */}
      {activeModule === "style-converter" && (
        <StyleConverter
      ...
      {/* Module 3: Manual Chord Transposer */}
      {activeModule === "transposer" && (
